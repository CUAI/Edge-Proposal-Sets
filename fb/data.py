...
class FBDataset(InMemoryDataset):
    def __init__(self, root = 'dataset/fb', transform=None, pre_transform=None):
        '''
            - name (str): name of the dataset
            - root (str): root directory to store the dataset folder
        '''
        self.root = root
        self._num_nodes = 22470
        super(FBDataset, self).__init__(self.root, transform, pre_transform)
    ...
    def processed_file_names(self):
        files = ['fb_data_processed.pt', 'fb_edge_set.pt', 'fb_split.pt']
        return files
    ...
    def process(self):
        ...
        data = pd.read_csv('fb/musae_facebook_edges.csv')
        ...
        with open(f"fb/musae_facebook_features.json", 'r') as f:
            j = json.load(f)
...
if __name__ == "__main__":
    dataset = FBDataset(transform=T.ToSparseTensor())
#     dataset = fbDataset()
