...
#         return torch.sparse.sum(common_neighbors, 1).to_dense(), None
...
def main():
    parser = argparse.ArgumentParser(description='email (GNN)')
...
#     data.adj_t = adj_t
    ...
    all_edges = torch.cat([pos_test_edge,neg_test_edge], dim=1)
    
    all_adamic = []
    for perm in tqdm(DataLoader(range(all_edges.size(1)), args.batch_size*10)):
        edges = all_edges[:, perm]
        common_neighbors = data.adj_t[edges[0]].to_torch_sparse_coo_tensor().mul(data.adj_t[edges[1]].to_torch_sparse_coo_tensor())
        degree = data.adj_t.sum(1)
        sparse_sizes  = data.adj_t[edges[0]].sparse_sizes()
        adamic = SparseTensor.from_edge_index(common_neighbors.indices(),  1./torch.log(degree[common_neighbors.indices()[1]]) * common_neighbors.values(), sparse_sizes = sparse_sizes)
        adamic = sparse_sum(adamic,1)
        adamic = torch.cat([edges.t().cuda(),adamic.unsqueeze(1)],1)
        all_adamic.append(adamic)

    all_adamic = torch.cat(all_adamic, 0)
   
    _, indices = adamic[:,2].sort(descending=True)
    sorted_test_edges = adamic[indices].cpu()
...
#         index_end = int(torch.tensor(run).float().exp())
        ...
        full_edge_weights = torch.ones(full_edge_index.size(1), dtype=torch.float)       
        adj_t = SparseTensor.from_edge_index(full_edge_index, full_edge_weights, sparse_sizes = data.adj_t.sparse_sizes()).t()
